...
var1 <- read.csv("var1.csv",row.names = 1,encoding = "utf-8")
var2 <- read.csv("var2.csv",row.names = 1,encoding = "utf-8")
...
double_cor <- function(data1, data2, wid, hei){
  ...
  for (i in 1:ncol(data1)) {
    for (j in 1:ncol(data2)) {
      ...
      res <- corr.test(data1[,i],data2[,j],method = 'spearman')
      r[j,i] <-  res$r
      p[j,i] <-  res$p
      colnames(r)[i] <- name1
      rownames(r)[j] <- name2
  ...
  pdf("1spearman.pdf",width = wid , height = hei)
